        ...
        const checkSystems = () => {
            ...
            if (updatedProfile.tower_progress) {
                ...
                if (!isToday(lastRegenTower)) {
                    ...
                    // We don't set profileChanged here because we might change it below, and we want to save once.
                    // Actually, the original code set profileChanged = true here. Let's keep it consistent.
                    profileChanged = true;
                }
            }

            // HP Regen & Offline Penalty Logic
            ...
            if (!isToday(lastHPRegen)) {
                ...
                // Offline Penalty Logic
                const lastMissionDate = updatedProfile.ultimo_dia_de_missao_concluida ? new Date(updatedProfile.ultimo_dia_de_missao_concluida) : new Date();
                const daysSinceLastMission = differenceInCalendarDays(now, lastMissionDate);

                if (daysSinceLastMission > 1) {
                    const hpLoss = 10 * (daysSinceLastMission - 1);
                    updatedProfile.hp_atual = Math.max(0, (updatedProfile.hp_atual || maxHP) - hpLoss);

                    toast({
                        variant: 'destructive',
                        title: 'Penalidade de Inatividade',
                        description: `Você perdeu ${hpLoss} HP por ficar ${daysSinceLastMission} dias sem completar missões.`
                    });

                    // Death Condition
                    if (updatedProfile.hp_atual <= 0) {
                        updatedProfile.hp_atual = maxHP; // Respawn with full HP

                        // Softened Penalty: Lose 25% of current XP instead of Level/All XP
                        const xpLoss = Math.floor(updatedProfile.xp * 0.25);
                        updatedProfile.xp = Math.max(0, updatedProfile.xp - xpLoss);

                        toast({
                            variant: 'destructive',
                            title: 'VOCÊ MORREU',
                            description: `A sua negligência custou caro. Você renasceu, mas perdeu parte da sua experiência (${xpLoss} XP).`
                        });
                    }
                } else if (updatedProfile.hp_atual < maxHP) {
